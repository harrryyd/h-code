...
export const make = Effect.fn("makeReviewService")(function* () {
  ...
  const deleteReviewComment: ReviewServiceShape["deleteReviewComment"] = Effect.fn(
    ...
    draftStore.get(input.threadId, input.prNumber).pipe(
      Effect.flatMap((option) => {
        ...
        const updatedComments = existingDraft.comments.filter((c) => c.id !== input.commentId);

        if (updatedComments.length === 0) {
          return draftStore
            .delete(input.threadId, input.prNumber)
            .pipe(Effect.as<ReviewDraft | null>(null));
        }
  ...
  const submitReview: ReviewServiceShape["submitReview"] = Effect.fn("ReviewService.submitReview")(
    (input) =>
      Effect.gen(function* () {
        const localDraftOption = yield* draftStore.get(input.threadId, input.prNumber);
        ...
        const localComments = draft.comments.filter((c) => c.author.login === "local");
