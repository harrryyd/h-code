/**
 * PR Review E2E — boots the REAL server in-process, clones a real GitHub
 * repo, fetches a PR, exercises the full review flow via WS RPC, then runs
 * the background agent and checks that the response does NOT contain the
 * "Agent process failed or timed out: undefined" bug.
 *
 ...
 *  10. agent        — changeRequest.runBackgroundAgent (real opencode run)
 *  11. verify       — assert no "undefined" in error messages
 *  12. verify       — check agent produced file changes in worktree
 *  13. cleanup      — interrupt server, delete temp dirs
 *
 * Usage:
 *   T3_TEST_PR_REPO=harrryyd/h-code T3_TEST_PR_NUMBER=121 node scratchpad/pr-review-agent-e2e.ts
 *
 * Requires:
...
const BOOTSTRAP_TOKEN = "e2e-pr-review-agent-bootstrap-token";
const STEP_TIMEOUT_SECONDS = 30;
const BOOT_TIMEOUT_SECONDS = 45;
const AGENT_TIMEOUT_SECONDS = 120; // 2 minutes max for fast model
...
const program = Effect.gen(function* () {
    ...
    yield* Effect.die(
      new Error(
        "Set T3_TEST_PR_REPO=owner/repo and T3_TEST_PR_NUMBER=<n> environment variables.\n" +
          "Example: T3_TEST_PR_REPO=harrryyd/h-code T3_TEST_PR_NUMBER=121 node scratchpad/pr-review-agent-e2e.ts",
      ),
    );
  }
  ...
  const tmpDir = yield* Effect.promise(() =>
    Fs.mkdtemp(PathMod.join(Os.tmpdir(), "t3-pr-review-agent-e2e-")),
  );
  ...
  const tempBranch = `t3-e2e-agent-${id("pr")}`;
  yield* step(
    "clone: fetch PR head",
    Effect.tryPromise(() =>
  ...
  const baseDir = yield* Effect.promise(() =>
    Fs.mkdtemp(PathMod.join(Os.tmpdir(), "t3-pr-review-agent-base-")),
  );
  ...
  const config: ServerConfigShape = {
    ...
    otlpServiceName: "t3-server-pr-review-agent-e2e",
    mode: "desktop",
    port,
    host: "127.0.0.1",
    cwd: process.cwd(),
        ...
        Effect.gen(function* () {
          ...
          yield* step(
            ...
            client["orchestration.dispatchCommand"]({
              ...
              commandId: id("cmd"),
              projectId,
              title: "e2e pr review agent project",
          ...
          yield* step(
            ...
            client["orchestration.dispatchCommand"]({
              ...
              threadId,
              projectId,
              title: "e2e pr review agent thread",
              modelSelection: {
                instanceId: "opencode",
          ...
          const commentBody = `e2e-agent smoke test comment — please ignore.`;
          const reviewComment = {
            id: commentId,
            file: commentFile,
          ...
          return { threadId, commentId, commentBody, commentFile, worktreePath, tmpDir };
        }),
      ),
  ...
  const { threadId, commentId, commentBody, commentFile, worktreePath, tmpDir: _tmpDir } = wsSetupResult;

  // ——————— Step 10: run background agent ———————
  console.log("\n  [agent] Running background agent via WS RPC (may take up to 10 min)...\n");
  const agentEvents = yield* step(
    ...
    ),
    {
      ...
      describe: (events: any[]) => {
        ...
        return done
          ? `done (${events.length} events, types=[${types}])`
          : error
            ? `error: ${error.message}`
            : `${events.length} events, types=[${types}]`;
      },
    },
  );
  ...
  // Log every event for diagnostic purposes
  for (const ev of agentEvents) {
    const prefix = `  [agent event] type=${ev.type}`;
    if (ev.type === "text") {
      console.log(`${prefix} content=${ev.content?.slice(0, 100)}`);
    } else if (ev.type === "status") {
      console.log(`${prefix} agentStatus=${ev.agentStatus}`);
    } else if (ev.type === "error") {
      console.log(`${prefix} message=${ev.message}`);
    } else if (ev.type === "detail") {
      console.log(`${prefix} title=${ev.title} content=${ev.content?.slice(0, 100)}`);
    }
  }

  // ——————— Step 11: assert no "undefined" in error messages ———————
  const errorEvent = agentEvents.find((e: any) => e.type === "error");
  const hasUndefinedError =
    errorEvent?.message?.includes("undefined");

  yield* step(
    "verify: no undefined in error message",
    Effect.sync(() => {
      if (hasUndefinedError) {
        throw new StepFailure(
          `BUG REPRODUCED: Agent error message contains "undefined": "${errorEvent.message}"`,
        );
      }
      if (errorEvent) {
        console.log(
          `  [verify] Agent produced error (but not the undefined bug): ${errorEvent.message}`,
        );
      } else {
        console.log(`  [verify] No error event produced by agent.`);
      }
    }),
    {
      describe: () =>
        hasUndefinedError
          ? `"undefined" in error message (BUG!)`
          : errorEvent
            ? `error present but no "undefined" in message`
            : "no error event",
    },
  );

  // ——————— Step 12: verify agent produced changes ———————
  const gitStatus = yield* step(
    "verify: agent file changes in worktree",
    Effect.tryPromise(() => shellRun(`git status --porcelain`, worktreePath)),
    {
      describe: (status: string) =>
        status.length > 0
          ? `changes: ${status.split("\n").slice(0, 3).join("; ")}...`
          : "NO CHANGES (agent may have skipped due to trivia or error)",
    },
  );
